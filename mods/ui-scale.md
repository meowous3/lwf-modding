---
title: UI Scale
repo: meowous3/lwf-ui-scale
dll: LwfUiScale.dll
summary: A UI Scale slider in Graphic settings, 15% to 200%.
gameVersion: "0.21.0"
screenshot: mods/ui-scale.png
screenshotAlt: The Graphic settings page with a UI Scale slider below Frame Rate Control
---

Adds a **UI Scale** row to Settings → Graphic. Drag the slider and let go; the whole interface
resizes.

Runs from 15% to 200%. The setting is kept in `BepInEx/config/dev.meow.lwfuiscale.cfg`.
